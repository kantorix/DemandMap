...
class Resources_Controller extends Admin_Controller {
    ...
    public function submit() {
		...
		$this->template->content = new View('admin/resources/edit');
        $material = new Material_Model();
	...
	//loads the view with the commentssection below
	 public function view($material_id=0) {
		...
		$this->template->content = new View('admin/resources/view');
        $material = new Material_Model();
		...
		if ($_POST){
			...
			if (resources::validate_comments($post)){ //passing validation
				...
				$talk->save();		
				url::redirect(url::site().'admin/resources/view/'.$material->id);
			}else{
    ...
    public function post($material_id=0, $saved = FALSE) {
		...
		if ($_POST){
			...
			if (resources::validate_materials($post)){ //passing validation
				...
				*/		
				url::redirect(url::site().'admin/resources/thanks');
			} else{
				// Repopulate the form fields
				$form = arr::overwrite($form, $post->as_array());
	...
	public function thanks(){
		$this->template->content = new View('admin/resources/submit_thanks');
	}
	
	/*public function upload($id=0) {
		$this->template->content = new View('admin/resources/upload');
	...
	public function getfile($material_id=0) {
		...
		//$this->template->content = new View('admin/resources/getfile');
		$db = new Database();
		$file = new File_Model();
