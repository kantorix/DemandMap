...
class User_Model extends Auth_User_Model {
  ...
  public static function custom_validate(array & $post, Auth $auth = NULL) {
    ...
    if (empty($post->user_id)) {
      $post->add_callbacks('username', array(
        'User_Model',
        'unique_value_exists'
      ));
    ...
    if (!$auth->logged_in('superadmin')) {
      $post->add_callbacks('role', array(
        'User_Model',
        'prevent_superadmin_modification'
      ));
    }
  ...
  public static function unique_value_exists(Validation $post, $field) {
    $exists = (bool) ORM::factory('user')->where($field, $post[$field])
      ->count_all();
    if ($exists) {
  ...
  */
  public function random_username() {
    while ($random = mt_rand(1000, mt_getrandmax())) {
      $find_username = ORM::factory('user')->where('username', $random)
        ->count_all();
      if ($find_username == 0) {
        return $random;
      }
  ...
  public function has_permission($permission) {
    ...
    static $superadmin;
    if (!isset($superadmin)) {
      $superadmin = ORM::factory('role', 'superadmin');
    }
